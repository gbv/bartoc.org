        ...
        <tr
          ...
          <td
            ...
            <div class="cc-button-group">
              ...
              <button
                :disabled="i >= editableSubjectEntries.length - 1"
                type="button"
                class="cc-button cc-button-secondary cc-button-icon"
                @click="moveEditableSubject(i, 1)">
                &#9660;
...
const subjects = computed(() =>
  ...
  // but keep mapped subjects so they can pass through to the parent unchanged.
...
const editableSubjectEntries = computed(() =>
  subjects.value
    .map((subject, index) => ({ subject, index }))
    .filter(({ subject }) => !isMappedSubject(subject)),
)

// An inheritance preview shows the complete source field. The local editor
// continues to hide subjects generated by mapping enrichment.
const displayedSubjects = computed(() =>
  props.readOnly
    ? subjects.value.map((subject, index) => ({ subject, index }))
    : editableSubjectEntries.value,
)
...
function removeEditableSubject(i) {
  const subjectIndex = editableSubjectEntries.value[i]?.index
...
// Reorder visible subjects using their indexes in the full list.
// Do not move hidden mapped subjects.
function moveEditableSubject(i, direction) {
  const target = i + direction
  if (target < 0 || target >= editableSubjectEntries.value.length) {
    return
  }

  const next = [...subjects.value]
  const fromIndex = editableSubjectEntries.value[i].index
  const toIndex = editableSubjectEntries.value[target].index
